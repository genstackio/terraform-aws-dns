...
resource "aws_route53_record" "dkim-01" {
  ...
  type    = "CNAME"
  ttl     = var.dkim_ttl
  records = ["${var.dkim_verification_id_01}.dkim.amazonses.com"]
}
resource "aws_route53_record" "dkim-02" {
  ...
  type    = "CNAME"
  ttl     = var.dkim_ttl
  records = ["${var.dkim_verification_id_02}.dkim.amazonses.com"]
}
resource "aws_route53_record" "dkim-03" {
  ...
  type    = "CNAME"
  ttl     = var.dkim_ttl
  records = ["${var.dkim_verification_id_03}.dkim.amazonses.com"]
}