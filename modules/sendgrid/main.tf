...
resource "aws_route53_record" "s03" {
  ...
  type    = "CNAME"
  ttl     = var.ttl
  records = ["${var.value_03}.wl232.sendgrid.net"]
}
resource "aws_route53_record" "dkim-01" {
  ...
  type    = "CNAME"
  ttl     = var.dkim_ttl
  records = ["s1.domainkey.${var.dkim_verification_id_01}.wl232.sendgrid.net"]
}
resource "aws_route53_record" "dkim-02" {
  ...
  type    = "CNAME"
  ttl     = var.dkim_ttl
  records = ["s2.domainkey.${var.dkim_verification_id_02}.wl232.sendgrid.net"]
}
